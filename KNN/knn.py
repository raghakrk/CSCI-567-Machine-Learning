...
class KNN:
    ...
    def predict(self, features):
        ...
        yp=[]
        for i in features:
            nn=self.get_k_neighbors(i)
            most_common,num_most_common = Counter(nn).most_common(1)[0]
            yp.append(most_common)
        return (yp)
    ...
    def get_k_neighbors(self, point):
        ...
        for i in self.feature:
            dist.append(self.distance_function(i,point))
        dataset=np.concatenate((np.asarray(dist).reshape(len(dist),1),np.asarray(self.label).reshape(len(self.label),1)),axis=1)
        sort_label=sorted(dataset,key=lambda lab:lab[0] )
        for i in sort_label[:self.k]:
            label_no.append(i[1])
        return (label_no)
        raise NotImplementedError
